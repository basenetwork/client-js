...
base.core = {
    ...
    ex_getAuthorInfo: function(data, fn) {
        this.getAuthorInfo(data, fn);
    },
    ...
    requestData: function(req, fn) {
        ...
        this.requestToNodesBySegment(req.segment, uri, null, function(err, response) {
            if(err) return fn(err);
            ...
            res && res.split('\n').forEach(function(line){
                ...
                if(!pack) return err = 'Incorrect data';
                if(!pack.data) return; // deleted
                // check access (todo: to server???)
                if(accessForAll || accessOnlyReg && pack.author.signed || access.indexOf(pack.author) > -1) {
                    packs.push(pack);
                }
                // else  return 'Author is not registered';
            });
            if(packs.length || !err) fn(null, packs);
            else return fn(err);
        });
    },
    ...
    postData: function(req, fn) {
        ...
        var uid     = _.str(req.uid);           if(!uid) return fn("Empty uid");
        var pos     = _.str(req.pos || "");     if(pos.length > 20) return fn("Position is too long");
        ...
        if(req.recipient) data = cert.encrypt(data, req.recipient);
    ...
    getAuthorInfo: function(aid, fn) {
        // todo: use queue (for the same requests)
        if(!aid) return fn("Empty author-param");
        ...
        var _authors = this._authors || (this._authors = {});
        var info = _authors[aid] || _.lsCache.get(aid);
        ...
        var search = function(ring) {
            ...
            this.requestData({
                ...
                info = packs[0].data;
                _.lsCache.set(aid, _authors[aid] = info);
                fn(null, info);
            });

        }.bind(this);
        search(1); // begin from ring-1
    }
};
