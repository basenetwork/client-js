...
base.API.v0 = {
    ...
    //-------- Other Information ---------
    /**
     * return certificate-information from registrar
     * @param certificate  public-certificate
     * @param callback  Function(error, certInfo)
     * @returns { name: "user-name", domain: "domain-name" }
     */
    getCertificateInfo: function(certificate, callback) {
        if(certificate.cert) certificate = certificate.cert;
        return base.site.requestToCore("getCertificateInfo", certificate, callback);
    },

    /**
     * return user information from user
     * @param certificate    aid or public-certificate
     * @param callback  Function(error, userInfo)
     * @returns { icon:"icon-uri", name:"user-name" }
     */
    getUserInfo: function(certificate, callback) {
        if(certificate.cert) certificate = certificate.cert;
        return base.site.requestToCore("getUserInfo", certificate, callback);
    }
};
